...
namespace TimeTableWebAPI.Services.SubjectService
{
    public class SubjectMockService : ISubjectService
    {
        ...
        /*
        public async Task<ApiReturnValue<Lecturers>> GetSubjects(string subjectName)
        {
            await CreateSampleSubjects();

            var value = (from s in ListSubject where s.SubjectDescription.ToLower().Contains(subjectName.ToLower()) select s).ToList();

            return value;
        }
        public async Task<ApiReturnValue<Lecturers>> UpdateSubject(string subjectId, Subject subject)
        {
